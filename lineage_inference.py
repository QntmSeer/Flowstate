...
class LineageVariationalEM(VariationalEM):
    """
    Extends VariationalEM with a Clonal Regularization Penalty.

    During the continuous E-step, each cell's smoothed latent mean mu_z is
    pulled towards the centroid of its clone group. This enforces the biological
    constraint that clonally related cells (sharing a barcode) must trace
    contiguous paths through the latent state space.
    """

    def fit_with_barcodes(self, x: jnp.ndarray, barcodes: jnp.ndarray,
                          lambda_clonal: float = 1.0, max_iter: int = 10):
        """
        Args:
            x: (T, D_x) Observed gene expression matrix.
            barcodes: (T,) Integer lineage barcode ID per cell.
            lambda_clonal: Interpolation weight [0, 1] towards the clone centroid.
                           0 = no regularization, 1 = fully collapsed to centroid.
            max_iter: Number of EM iterations.

        Returns:
            expected_s: (T, K) Smoothed discrete state probabilities.
            mu_z_reg: (T, D_z) Regularized smoothed latent means.
        """
        T = x.shape[0]
        ...
        for i in range(max_iter):
            # --- Continuous E-Step (Kalman Smoother) ---
            if i == 0:
            ...
            # --- Clonal Regularization ---
            # For each clone, compute its centroid and pull member cells towards it.
            # Implemented via vmap over clone IDs, then summed to reconstruct the
            # full regularized trajectory.
            ...
            print(f"Lineage EM Iter {i+1}/{max_iter} completed.")
